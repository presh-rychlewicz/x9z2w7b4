...
export function Dashboard({ stories, onSelectStory, completedStoryIds }: DashboardProps) {
  ...
  }, [stories, searchQuery, selectedDifficulty, selectedProgress, completedStoryIds])

  const getDifficultyStyle = (diff: string) => {
    if (diff === 'Beginner') return { bg: 'rgba(46,125,50,0.1)', color: '#2e7d32', border: 'rgba(46,125,50,0.3)' }
    if (diff === 'Intermediate') return { bg: 'rgba(237,108,2,0.1)', color: '#ed6c02', border: 'rgba(237,108,2,0.3)' }
    return { bg: 'rgba(156,39,176,0.1)', color: '#9c27b0', border: 'rgba(156,39,176,0.3)' }
  }
    ...
    <Box>
      ...
      <Box sx={{ mb: 5, textAlign: 'center' }}>
        ...
        >
          ...
          <Typography
            variant="caption"
            color="primary"
            sx={{ fontWeight: '700', fontFamily: 'Inter, system-ui, sans-serif', letterSpacing: 1 }}
          >
      ...
      {filteredStories.length > 0 ? (
              ...
              >
                  ...
                  <Box>
                    ...
                    >
                      <Typography
                        variant="h5"
                        component="h2"
                        sx={{ fontWeight: '700', fontFamily: 'Inter, system-ui, sans-serif', letterSpacing: '-0.01em' }}
                      >
                        {story.title}
                      </Typography>
                      ...
                      {isCompleted ? (
                        <Chip
                          icon={<CheckCircle style={{ fontSize: 14, color: '#2e7d32' }} />}
                          label="Completed"
                          size="small"
                          color="success"
                          variant="outlined"
                          sx={{ fontWeight: '700', fontSize: '0.75rem', fontFamily: 'Inter, system-ui, sans-serif' }}
                        />
                      ) : (
                        <Chip
                          icon={<RadioButtonUnchecked style={{ fontSize: 14, color: '#666' }} />}
                          label="Unread"
                          size="small"
                          variant="outlined"
                          sx={{ fontWeight: '600', fontSize: '0.75rem', fontFamily: 'Inter, system-ui, sans-serif' }}
                        />
                      )}
                    </Box>
                    ...
                    <Typography
                      variant="body1"
                      color="text.secondary"
                      sx={{ mb: 2, fontFamily: 'Inter, system-ui, sans-serif', lineHeight: 1.5, maxWidth: 800 }}
                    >
                      {story.synopsis}
                    </Typography>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 2.5 }}>
                      {[
                        { icon: <AccessTime sx={{ fontSize: 16 }} />, label: `${story.readingTimeMin} min read` },
                        { icon: <Layers sx={{ fontSize: 16 }} />, label: `${story.wordCount} words` },
                        { icon: <MenuBook sx={{ fontSize: 16 }} />, label: `${story.glossary.length} vocab words` },
                      ].map(({ icon, label }) => (
                        <Box
                          key={label}
                          sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 0.8, color: 'text.secondary' }}
                        >
                          {icon}
                          <Typography variant="caption" sx={{ fontWeight: '600', fontFamily: 'Inter, system-ui, sans-serif' }}>
                            {label}
                          </Typography>
                        </Box>
                      ))}
                ...
                >
                  <Button
                    variant="text"
                    endIcon={
                      <ChevronRight className="chevron-icon" sx={{ transition: 'transform 0.2s, color 0.2s' }} />
                    }
                    sx={{
                      textTransform: 'none',
                      fontWeight: '700',
        ...
        >
          ...
          <Typography
            variant="h6"
            sx={{ fontWeight: '700', fontFamily: 'Inter, system-ui, sans-serif', mb: 1 }}
          >
          ...
          <Button
            variant="outlined"
            onClick={() => { setSearchQuery(''); setSelectedDifficulty('All'); setSelectedProgress('All') }}
            sx={{ borderRadius: 2, textTransform: 'none', fontFamily: 'Inter, system-ui, sans-serif', fontWeight: '600' }}
          >
            Reset Filters
          </Button>
        </Paper>
